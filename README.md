...
- 開発パートナーと強調して WP テーマ開発を行うときに標準的な構成として提案できること
...
そのとき、基本的にはサーバーやインフラの管理は弊社で行っているので、その標準的な構成に合わせた構成を提供することで、環境の統一を図り、開発効率を向上させることができ流と考えています。

### Bedrock 採用の理由

一般的な WordPress 構成とこの WP Standard で最も異なる点は Bedrock の採用だと思います。
...
つまり、WordPress テーマ開発では、テーマ (`wp-content/themes/*`) のみをバージョン管理することが一般的に行われていますが、実際の運用では、WordPress コアのバージョンやプラグイン構成、そして機密情報など、テーマだけでは完結しないものも含めて運用されるため、 それらも含めて管理しやすいようにしたい、という動機で採用しているものとなります。

ただ、あくまでもこの Bedrock は WordPress 構成のボイラープレートに過ぎませんので、この中に `themes` ディレクトリは含まれますし、
Bedrock を使っているから必ず特殊なテーマ開発やプログラミングをしなければならない、というものではありません。
Bedrock ではない環境で開発したテーマも、Bedrock の `themes` ディレクトリに設置すれば基本的には動作します。

ですので、テーマ開発という点では、例えば開発を LAMP / MAMP などで行っていただくなど普段通りの慣れた環境で開発をしていただき、Git に commit / push するときにテーマのコードのみを Bedrock 構成に揃えるということも可能です。

Bedrock をご存知なかった場合に詳細を知裏たい場合には、公式サイトをご覧いただくのが一番とは思いますが、例えば以下のページのような外部サイトでも紹介されているものが多くあるので、まずは軽くそういったサイトで概要を掴んでいただくのも良いかと思います。
...
こういった多くのメリットがあるため、開発環境の構築については Dev Container を利用することを前提としているのですが、これを利用するのも、テーマ開発という点では必須ではありません。
...
それだけで、`.devcontainer` に格納された設定情報にしたがって、自動的に Docker コンテナ内の開発環境が起動します。

注意事項:

> **Note**
> - 初回起動時には、Docker イメージのダウンロードなども行うので、上記実行に多少時間がかかります。エラーがなければ待っていれば終了します
> - 初回起動時に、MySQL コンテナの初期化処理に時間がかかりエラーが発生する場合があります。そのときは、**Dev-Containers: Rebuild Container** を実行してコンテナを再ビルドすること
> - 稀に、再構築などを実行してもどうしても復旧しない場合があります。そのときな、データベースのデータは初期化されますが、Docker ボリュームの削除が有効なことがあります。その際には `docker volume ls` や Docker Desktop アプリケーションのダッシュボードからボリュームを特定し、個別に削除してください
...
テーマ開発のコードスタイルは [PSR2](https://www.php-fig.org/psr/psr-2/) のコーディングスタイルに準拠しています。

> **Note**
> WordPress のコーディング規約ではないので注意。
> 詳細な設定は `cms/phpcs.xml` を参照ください。

コードスタイルについては、Dev Container を利用して開発する場合は、必要パッケージと拡張機能がデフォルトインストールされるので、エディタ上の指示に従えば良いです。

このスタイルだと開発しにくいなどあればご相談ください。
